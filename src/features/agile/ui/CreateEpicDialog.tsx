...
export function CreateEpicDialog({
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      await epicService.create(projectId, {
        title: title.trim(),
        status,
        ...
        versionIdsJson: JSON.stringify(versionIds),
      });
      onCreated();
      onClose();
    } catch (err) {
    ...
    >
      {step === 0 && (
        <UsForm>
          ...
          />
          <UsField label="Versions" required hint="Versions where this epic will be delivered.">
            <div className="us-version-picker">
              {versions.length === 0 && (
                <p className="text-sm text-[var(--trello-text-muted)]">Create a version first.</p>
              )}
              {versions.map((v) => (
                <label key={v.id} className="us-version-picker__item">
                  <input
                    type="checkbox"
                    checked={versionIds.includes(v.id)}
