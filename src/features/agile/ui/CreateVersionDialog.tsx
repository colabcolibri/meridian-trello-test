...
import { stringifyChecklist, stringifyStringList } from "../../../domain/meridianJson";
import { invokeErrorMessage } from "../../../lib/errors";
import { validateName } from "../../../domain/validators";
import { versionService } from "../versionService";
import { ChecklistEditor, StringListEditor } from "./ListFieldEditor";
...
const STEPS: WizardStep[] = [
  ...
  },
  {
    id: "scope",
    label: "Scope",
    hint: "Included and Explicitly out — what is in and what waits.",
  },
  {
    id: "golive",
    label: "Go-live",
...
export function CreateVersionDialog({
  ...
  const validateStep = (index: number): string | null => {
    ...
    if (index === 2) {
      if (included.map((s) => s.trim()).filter(Boolean).length < 1) {
        return "Included needs at least 1 referenced epic or story.";
      }
  ...
  const submit = async () => {
    ...
    try {
      await versionService.create(projectId, {
        ...
        includedJson: stringifyStringList(included),
  ...
  return (
    ...
    >
      ...
      {step === 2 && (
        <UsForm>
          <UsField
            label="Included in this version"
            required
            hint="EPIC-XX or US-YYYY — one line each."
          >
            <StringListEditor
              items={included}
              onChange={setIncluded}
              placeholder="EPIC-01 — capability X"
            />
          </UsField>
          <UsTextareaField
            id="v-out"
